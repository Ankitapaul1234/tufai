...
//         <CardTitle className="gradient-title text-4xl">Complete Your Profile
//         </CardTitle>
...
const OnboardingForm = ({ industries }) => {
  const [selectedIndustry, setSelectedIndustry] = useState(null);
  const [hydrated, setHydrated] = useState(false); // ✅ Fix for hydration issue
  const router = useRouter();

  useEffect(() => {
    setHydrated(true); // Ensures it only renders after hydration
  }, []);
  ...
  // ✅ Prevent mismatches by not rendering anything until hydrated
